...
class September extends Component {
  ...
  httpGetPosts() {
    ...
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/september.json`;
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/september/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        sepTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="september" className="month">
          <h1 className="month-header">SEPTEMBER 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.sepTotal}</span></h2>
          <br />
        </div>
      </div>
    );
