...
class December extends Component {
  ...
  httpGetPosts() {
    ...
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/december.json`;
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/december/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        decTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="december" className="month">
          <h1 className="month-header">DECEMBER 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.decTotal}</span></h2>
          <br />
        </div>
      </div>
    );
