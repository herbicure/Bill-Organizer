...
class May extends Component {
  ...
  httpGetPosts() {
    ...
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/may.json`;
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/may/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        mayTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="may" className="month">
          <h1 className="month-header">MAY 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.mayTotal}</span></h2>
          <br />
        </div>
      </div>
    );
