...
class November extends Component {
  ...
  httpGetPosts() {
    ...
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/november.json`;
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/november/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        novTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="november" className="month">
          <h1 className="month-header">NOVEMBER 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.novTotal}</span></h2>
          <br />
        </div>
      </div>
    );
