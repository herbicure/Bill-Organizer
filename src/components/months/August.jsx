...
class August extends Component {
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/august/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        augTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="august" className="month">
          <h1 className="month-header">AUGUST 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.augTotal}</span></h2>
          <br />
        </div>
      </div>
    );
