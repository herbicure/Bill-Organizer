...
class July extends Component {
  ...
  httpGetPosts() {
    ...
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/july.json`;
  ...
  httpRemovePost(id) {
    let userId = firebase.auth().currentUser.uid;
    const url = `https://monthly-bill-organizer.firebaseio.com/users/${userId}/2016/july/${id}.json`;
  ...
  calculateTotal() {
    ...
    this.state.billPosts.forEach((bill) => {
      ...
      this.setState({
        julTotal: updatedTotal.toFixed(2),
      });
    });
  }
  render() {
    return (
      <div>
        <br />
        <div id="july" className="month">
          <h1 className="month-header">JULY 2016</h1>
          <BillPost
          ...
          />
          <br />
          <h2>TOTAL: <span id="tot"> ${this.state.julTotal}</span></h2>
          <br />
        </div>
      </div>
    );
